  ...
  testGroup
    ...
    [ testCase "Simple function" $
      ...
      testCase
        ...
        -- taken from the book(replacing Nat=>Top, Even=>Nat)
        -- µX.Top→(Nat × X) <: µX.Nat→(Top × X)
        $ assertEqual
          []
          ( subtype
              (TyRecursive "X" $ TyArrow TyTop (TyRecord $ fromList [("1", TyNat), ("2", TyVar 0 1)]))
              (TyRecursive "X" $ TyArrow TyNat (TyRecord $ fromList [("1", TyTop), ("2", TyVar 0 1)]))
          )
          True
    ]